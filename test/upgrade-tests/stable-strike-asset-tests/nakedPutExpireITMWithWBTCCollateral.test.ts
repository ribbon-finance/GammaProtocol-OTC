...
contract('Naked Put Option with WBTC collateral and stable strike asset expires Itm flow', ([accountOwner1, buyer]) => {
  let expiry: number
  ...
  const wbtcStartPrice = 30000
  const wbtcExpiryPrice = 10000
  const wethStartPrice = 1500
  const optionsAmount = 10
  const collateralAmountInUSD = optionsAmount * strikePrice
  const collateralAmountInCollateralAsset = collateralAmountInUSD / wbtcStartPrice
  ...
  before('set up contracts', async () => {
    ...
    // setup stableStrikeAsset, weth and wbtc
    ...
    // mint wbtc to user
    const accountOwner1Wbtc = createTokenAmount(10 * collateralAmountInCollateralAsset, wbtcDecimals)
    await wbtc.mint(accountOwner1, accountOwner1Wbtc)

    // have the user approve all the wbtc transfers
    await wbtc.approve(marginPool.address, accountOwner1Wbtc, { from: accountOwner1 })
  ...
  describe('Integration test: Close a naked put after it expires ITM', () => {
    ...
    const scaledCollateralAmount = createTokenAmount(collateralAmountInCollateralAsset, wbtcDecimals)
    ...
    before('Seller should be able to open a short put option', async () => {
      ...
      await oracle.setRealTimePrice(weth.address, scaleBigNum(wethStartPrice, 8))
      await oracle.setRealTimePrice(wbtc.address, scaleBigNum(wbtcStartPrice, 8))
    ...
    it('Seller: close an ITM position after expiry', async () => {
      ...
      const ownerWbtcBalanceBefore = new BigNumber(await wbtc.balanceOf(accountOwner1))
      const marginPoolWbtcBalanceBefore = new BigNumber(await wbtc.balanceOf(marginPool.address))
      ...
      const scaledWBTCPrice = createTokenAmount(wbtcExpiryPrice) // Simulates WBTC/USD price dumping
      ...
      await oracle.setExpiryPriceFinalizedAllPeiodOver(wbtc.address, expiry, scaledWBTCPrice, true)
      ...
      const collateralPayout = collateralAmountInCollateralAsset - (strikePriceChange * optionsAmount) / wbtcExpiryPrice // Since WBTC/USD now 10000 from 30000, payout in WBTC triples
      const scaledPayout = createTokenAmount(collateralPayout, wbtcDecimals)
      ...
      const ownerWbtcBalanceAfter = new BigNumber(await wbtc.balanceOf(accountOwner1))
      const marginPoolWbtcBalanceAfter = new BigNumber(await wbtc.balanceOf(marginPool.address))
      ...
      assert.equal(ownerWbtcBalanceBefore.plus(scaledPayout).toString(), ownerWbtcBalanceAfter.toString())
      assert.equal(marginPoolWbtcBalanceBefore.minus(scaledPayout).toString(), marginPoolWbtcBalanceAfter.toString())
    ...
    it('Buyer: redeem ITM put option after expiry', async () => {
      ...
      // Keep track of balances before
      const ownerWbtcBalanceBefore = new BigNumber(await wbtc.balanceOf(buyer))
      const marginPoolWbtcBalanceBefore = new BigNumber(await wbtc.balanceOf(marginPool.address))
      ...
      // keep track of balances after
      const ownerWbtcBalanceAfter = new BigNumber(await wbtc.balanceOf(buyer))
      const marginPoolWbtcBalanceAfter = new BigNumber(await wbtc.balanceOf(marginPool.address))
      const ownerOtokenBalanceAfter = new BigNumber(await ethPut.balanceOf(buyer))
      const oTokenSupplyAfter = new BigNumber(await ethPut.totalSupply())

      const payout = (strikePriceChange * optionsAmount) / wbtcExpiryPrice // Since WBTC/USD now 10000, payout in WBTC triples
      const scaledPayout = createTokenAmount(payout, wbtcDecimals)
